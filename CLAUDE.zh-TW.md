...
這十一個 subagents 組成一個協作團隊，用 `Agent` 工具呼叫（`subagent_type` 填寫下方名稱）。

| Agent | 呼叫名稱 | 使用時機 |
|-------|---------|---------|
| 找碴專家 | `critic` | 程式碼審查、安全審查、計畫審查、部署前檢查、🔴 漏洞 inline PoC 驗證 |
...
| 要寫程式碼前安全審查 / 懷疑有漏洞 | `critic`（含安全審查 + inline PoC 驗證） |
...
安全審查：critic 找漏洞 + inline PoC 驗證 → 修復 or 發 PR
...
- **十一人專家團隊架構** 是多個月實戰迭代的產物，針對委派式 AI coding 最能穩定產出的配置。
